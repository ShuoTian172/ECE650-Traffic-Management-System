 ...
 * @param result
 */
void cnf_sat_vc(const int &v, vector<int> *&_adj, vector<int> &result) {
  result.clear();
  int n = v; // node number
...
void approx_vc_1(const int &v, vector<int> *&_adj, vector<int> &result) {
  int n = v; // node number
...
void approx_vc_2(const int &v, vector<int> *&_adj, vector<int> &result) {
  int n = v;
  // deep copy array
  vector<int> *adj = new vector<int>[v];
  deep_copy_vector(_adj, adj, v);
...
void handle_input(string line, int &v, vector<int> *&adj) {
  ...
  if (line[0] == 'E') {
    ...
    std::thread threadCNF(cnf_sat_vc, std::ref(v), std::ref(adj), std::ref(result_cnf));
    std::thread threadA1(approx_vc_1, std::ref(v), std::ref(adj), std::ref(result_a1));
    std::thread threadA2(approx_vc_2, std::ref(v), std::ref(adj), std::ref(result_a2));
    threadCNF.join();
    threadA1.join();
    threadA2.join();

    sort_print_result(result_cnf, "CNF-SAT-VC: ");
    sort_print_result(result_a1, "APPROX-VC-1: ");
    sort_print_result(result_a2, "APPROX-VC-2: ");
